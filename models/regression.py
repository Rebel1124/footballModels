...
def _scale_to_odds(team_strengths, min_odd=0.5, max_odd=3.0, center_val=1.0):
    """
    ...
    min_odd, max_odd : float
        Minimum and maximum allowed odds values (now 0.5 to 3.0)
    center_val : float
        ...
        for match_idx_position, match_idx in enumerate(season_indices):
            ...
            if len(prev_season_matches) >= min_matches_needed:
                ...
                try:
                    ...
                    # Scale home advantage to 0.5-3.0 range (modified from original 0.5-5.0)
                    # In football, home advantage is typically 0.3-0.5 goals
                    if raw_home_advantage > 0:
                        # Positive home advantage (normal case)
                        # Map 0-2 goal advantage to 1.0-3.0 range
                        home_advantage = 1.0 + min(2.0, raw_home_advantage)
                    else:
                        # Negative or zero home advantage (rare case)
                        # Map -2-0 goal disadvantage to 0.5-1.0 range
                        home_advantage = 1.0 + max(-0.5, raw_home_advantage / 2)
                        
                    # Ensure home advantage is within bounds
                    home_advantage = min(3.0, max(0.5, home_advantage))
                ...
                try:
                    ...
                    # Convert to odds-style scaling with new min_odd=0.5, max_odd=3.0
                    team_strengths_twl = _scale_to_odds(team_strengths_log, min_odd=0.5, max_odd=3.0)
                    
                except Exception as e:
                    print(f"  Error fitting logistic regression: {e}")
                
                # Scale linear regression strengths to odds format with new min_odd=0.5, max_odd=3.0
                team_strengths_twr = _scale_to_odds(team_strengths_twr, min_odd=0.5, max_odd=3.0)
...
# Example usage
df_with_twr = add_time_weighted_ratings(file_path='nfd_data.xlsx', num_recent_games=6, time_decay_half_life=6, output_file='regression_0.5_3.0.xlsx')