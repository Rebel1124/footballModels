...
MODEL_DATABASE_ID = os.getenv('APPWRITE_CURRENT_MODEL_DB_ID')
MODEL_COLLECTION_ID = os.getenv('NFD_MODEL_CURRENT_SEASON_COLLECTION_ID')
...
def update_database(dataframe):
    """
    Optimized function to update existing records in Appwrite database.
    Specifically designed to avoid timeouts by using efficient batch processing.
    """
    ...
    print(f"Preparing to update {len(df)} records")
    
    # OPTIMIZATION 1: Fast data preparation with vectorized operations
    ...
    # OPTIMIZATION 2: Group updates by match_id for efficiency
    updates_by_match_id = {}
    
    # Convert DataFrame to dictionaries with proper handling of data types
    for _, row in df.iterrows():
        ...
        for key, value in row.items():
            # Skip system columns
            if key.startswith('$') or key == 'index':
                continue
            ...
            if pd.isna(value) or value is None:
                ...
                if isinstance(value, (np.integer, np.int64)):
                    ...
                    doc_data[key] = bool(value)
                else:
                    doc_data[key] = value
                    
        updates_by_match_id[match_id] = doc_data
    
    # OPTIMIZATION 3: Process updates in small batches with efficient document lookup
    try:
        # Constants for batch processing
        MATCH_ID_BATCH_SIZE = 20  # Number of match_ids to query at once
        UPDATE_BATCH_SIZE = 5     # Number of documents to update at once
        MAX_FUNCTION_TIME = 900    # Maximum function runtime in seconds
        
        # Split match_ids into batches for querying
        all_match_ids = list(updates_by_match_id.keys())
        
        print(f"Processing updates for {len(all_match_ids)} unique match IDs")
        
        # Process in chunks to avoid memory issues and timeouts
        for i in range(0, len(all_match_ids), MATCH_ID_BATCH_SIZE):
            # Check if we're approaching timeout
            elapsed_time = time.time() - start_time
            if elapsed_time > (MAX_FUNCTION_TIME * 0.9):
                print(f"⚠️ Approaching time limit after {elapsed_time:.1f}s. Processed {success_count + error_count} records.")
                break
                
            # Get current batch of match_ids
            batch_match_ids = all_match_ids[i:i + MATCH_ID_BATCH_SIZE]
            
            # OPTIMIZATION 4: Efficient document lookup - query by match_ids
            try:
                # Find documents matching these match_ids
                response = databases.list_documents(
                    database_id=MODEL_DATABASE_ID,
                    collection_id=MODEL_COLLECTION_ID,
                    queries=[
                        Query.equal("match_id", batch_match_ids),  # Query for multiple IDs at once
                        Query.select(['$id', 'match_id']),         # Only retrieve the fields we need
                        Query.limit(len(batch_match_ids))          # Limit to expected number
                    ]
                )
                
                # Extract document IDs mapped to match_ids
                docs_to_update = []
                for doc in response['documents']:
                    if 'match_id' in doc and doc['match_id'] in updates_by_match_id:
                        docs_to_update.append({
                            'document_id': doc['$id'],
                            'match_id': doc['match_id'],
                            'data': updates_by_match_id[doc['match_id']]
                        })
                
                # OPTIMIZATION 5: Fast batch updates with minimal logging
                for j in range(0, len(docs_to_update), UPDATE_BATCH_SIZE):
                    update_batch = docs_to_update[j:j + UPDATE_BATCH_SIZE]
                    
                    # Perform updates in parallel (as much as Appwrite allows)
                    update_promises = []
                    for update in update_batch:
                        try:
                            ...
                            error_count += 1
                            # Minimal error logging to avoid slowing down the process
                            if error_count <= 5:  # Only log first few errors
                                print(f"Error updating {update['match_id']}: {str(e)[:100]}...")
                
                # Log progress only occasionally
                if i % (MATCH_ID_BATCH_SIZE * 2) == 0 or i == 0:
                    elapsed = time.time() - start_time
                    percent_complete = (i + len(batch_match_ids)) / len(all_match_ids) * 100
                    print(f"Progress: {percent_complete:.1f}% - Successful updates: {success_count}, Errors: {error_count}, Time: {elapsed:.1f}s")
                
            except AppwriteException as e:
                print(f"Error during batch query: {str(e)[:150]}...")
                # Continue with next batch despite errors
        
        # Final status report
        total_time = time.time() - start_time
        print(f"Database update completed in {total_time:.2f}s")
        print(f"Successful updates: {success_count}, Failed updates: {error_count}")
        ...
        if success_count > 0:
            print(f"Partial success: {success_count} records updated before error")
            return True
        return False
...
#     # OPTIMIZATION 1: Only process matches that need updating -- I removed this because I want to update all matches in the dataframe.
