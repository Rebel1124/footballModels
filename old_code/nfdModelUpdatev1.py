...
def main(context):
    ...
    MODEL_DATABASE_ID = os.environ['APPWRITE_MODEL_DB_ID']
    NFD_MODEL_COLLECTION_ID = os.environ['NFD_MODEL_COLLECTION_ID']
    ...
    def update_database(dataframe):
        ...
        if 'status' in df.columns:
            ...
            context.log(f"Filtered to {len(df)} relevant matches for updating")
        ...
        context.log(f"Processing {len(df)} records for database update")
        ...
        try:
            ...
            context.log(f"Found {len(existing_docs)} existing documents in the database")
            ...
            if to_create:
                context.log(f"Creating {len(to_create)} new documents in batches")
                        ...
                        while retries < MAX_RETRIES:
                            try:
                                ...
                                if retries >= MAX_RETRIES:
                                    ...
                                    context.log(f"Failed to create after {MAX_RETRIES} attempts: {e}")
                                else:
                                    time.sleep(0.2)  # Shorter pause
                    
                    create_end_time = time.time()
                    if i % (BATCH_SIZE * 5) == 0:  # Log less frequently
                        context.log(f"Created batch {i//BATCH_SIZE + 1}/{(len(to_create)-1)//BATCH_SIZE + 1} in {create_end_time - create_start_time:.2f}s")
            ...
            if elapsed_time > (MAX_FUNCTION_TIME * 0.7) and len(to_update) > 50:
                ...
                context.log(f"Running low on time ({elapsed_time:.1f}s used). Prioritizing important updates.")
            ...
            if to_update:
                context.log(f"Updating {len(to_update)} existing documents in batches")
                ...
                for i in range(0, len(to_update), BATCH_SIZE):
                    ...
                    if i % (BATCH_SIZE * 10) == 0:  # Log much less frequently
                        context.log(f"Updating batch {i//BATCH_SIZE + 1}/{(len(to_update)-1)//BATCH_SIZE + 1}")
            ...
            context.log("Database update completed")
            return True
            
        except AppwriteException as e:
            context.log(f"Critical database error: {e}")
            return False
