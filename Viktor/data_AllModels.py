...
# MODEL_DATABASE_ID = os.getenv['APPWRITE_MODEL_DB_ID']
# NFD_MODEL_COLLECTION_ID = os.getenv['NFD_MODEL_COLLECTION_ID']
...
def _prepare_home_xg_regression_data(matches_df):
    ...
    for col in ['home_team_goals_scored_average', 'away_team_goals_conceded_average']:
        ...
        if valid_matches[col].isna().any():
            ...
            print(f"  Filling {valid_matches[col].isna().sum()} NA values in {col} with mean: {col_mean:.3f}")
    ...
    if len(valid_matches) > 1:
        ...
        if abs(correlation) > 0.997:  # Very high correlation
            print(f"  Warning: Features are highly correlated (r={correlation:.4f}), adding noise")
...
def calculate_massey_expected_goals(home_team, away_team, team_ratings, home_advantage=0.3, league_avg_goals=1.3):
    """
...
def add_massey_ratings(dataframe, num_recent_games=6, min_home_advantage=0.3, home_advantage_factor=1.3):
    """
    ...
    for i, season in enumerate(seasons):
        ...
        season_length = len(season_indices)
        calculation_interval = max(num_recent_games, int(season_length / max(1, len(teams_in_season))))
        ...
        for match_idx_position, match_idx in enumerate(season_indices):
            ...
            # Check if we should recalculate ratings
            enough_matches = len(prev_season_matches) >= min_matches_needed
            enough_new_matches = match_idx_position - last_calculation_index >= calculation_interval
            is_last_match = match_idx_position == len(season_indices) - 1
            
            if enough_matches and (enough_new_matches or is_last_match):
                print(f"  Calculating Massey ratings at match {match_idx_position+1}/{len(season_indices)}")
                ...
                try:
                    ...
                    for team in teams_in_season:
                        ...
                        if len(team_matches) > 0:
                            ...
                            for _, match in recent_team_matches.iterrows():
                                ...
                                if match_tuple not in [(m['home_team'], m['away_team'], 
                    ...
                    if len(teams_to_rate) >= 3 and len(recent_matches_data) >= 3:
                        ...
                        A, b, team_to_idx = _build_massey_matrix(massey_df, teams_to_rate)
                        ...
                        if A.shape[0] > 0:
                            ...
                            print(f"  Successfully calculated ratings for {len(teams_to_rate)} teams")
                        else:
                            print(f"  Warning: Empty matrix for Massey calculation")
                    else:
                        print(f"  Not enough data: {len(teams_to_rate)} teams, {len(recent_matches_data)} matches")
            ...
            try:
                ...
                if len(season_df) > 0:
                    ...
                    if len(recent_season_matches) > 0:
                        league_avg_goals = (recent_season_matches['home_goals'].mean() + 
                                          recent_season_matches['away_goals'].mean()) / 2.0
                    else:
                ...
                # Calculate expected goals
                home_expected_goals, away_expected_goals = calculate_massey_expected_goals(
                    home_team, 
                    away_team, 
                    team_ratings, 
                    min_home_advantage,
