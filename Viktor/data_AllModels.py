...
from .db import get_matches_from_db
...
OUTPUT_FILENAME = "nfd_data_all_models.xlsx"
...
#     """Update the Appwrite database with the latest data, optimized for maximum efficiency."""

#     start_time = time.time()

#     # Make a copy to avoid modifying the original and use efficient data pipeline
#     df = dataframe.copy()

#     # OPTIMIZATION 1: Only process matches that need updating
#     # Use indexing to filter by match status if possible
#     if 'status' in df.columns:
#         # Only update upcoming or recently completed matches that need fresh predictions
#         statuses_to_update = ['upcoming', 'inprogress', 'complete']
#         df = df[df['status'].isin(statuses_to_update)]
#         print(f"Filtered to {len(df)} relevant matches for updating")

#     # OPTIMIZATION 2: Apply vectorized operations for data preparation
...
#     # Process integer columns all at once
#     integer_columns = [
...
#     # Create match_id efficiently
#     if 'match_id' not in df.columns and all(col in df.columns for col in ['date', 'homeID', 'awayID']):
#         df['match_id'] = df['date'].astype(str) + '_' + df['homeID'].astype(str) + '_' + df['awayID'].astype(str)

#     print(f"Processing {len(df)} records for database update")

#     # OPTIMIZATION 3: Find only the records that need to be updated in the database
#     try:
#         # First, get all existing match IDs in a single efficient query
#         existing_docs = {}

#         # OPTIMIZATION 4: Use cursor-based pagination (more efficient than offset)
#         limit = 100
#         queries = [
#             Query.limit(limit),
#             Query.select(['$id', 'match_id'])  # Only fetch the fields we need
#         ]

#         cursor = None
#         while True:
#             if cursor:
#                 current_queries = queries + [Query.cursor(cursor)]
#             else:
#                 current_queries = queries

#             response = databases.list_documents(
#                 database_id=MODEL_DATABASE_ID,
#                 collection_id=NFD_MODEL_COLLECTION_ID,
...
#             # Exit if no more documents or no cursor
#             if len(documents) < limit or 'cursor' not in response or not response['cursor']:
#                 break

#             cursor = response['cursor']

#         print(f"Found {len(existing_docs)} existing documents in the database")

#         # OPTIMIZATION 5: Incremental updates
#         # Create an array of document updates
#         to_create = []
#         to_update = []

#         # OPTIMIZATION 6: Process in chunks to avoid memory issues
#         chunk_size = 100  # Smaller chunk size for better memory management
#         for i in range(0, len(df), chunk_size):
#             chunk_df = df.iloc[i:i+chunk_size]

#             for _, row in chunk_df.iterrows():
#                 # Use a dict comprehension for cleaner and faster doc_data creation
#                 doc_data = {
#                     key: (0 if key in integer_columns and (pd.isna(value) or value is None) else
#                          False if key == 'rolling_stats_valid' and (pd.isna(value) or value is None) else
#                          0.0 if isinstance(value, float) and (pd.isna(value) or value is None) else
#                          "" if pd.isna(value) or value is None else
#                          value)
#                     for key, value in row.items()
#                 }

#                 # Skip if no match_id
#                 match_id = doc_data.get('match_id')
#                 if not match_id:
#                     continue

#                 # Check if document exists - add to appropriate list
#                 if match_id in existing_docs:
#                     to_update.append((existing_docs[match_id], doc_data))
#                 else:
#                     to_create.append(doc_data)

#         # OPTIMIZATION 7: Use parallelism for database operations
#         # Unfortunately, Appwrite functions don't support true parallelism yet
#         # but we can optimize batch size and retries
#         BATCH_SIZE = 5  # Smaller batch size to prevent timeouts
#         MAX_RETRIES = 2  # Fewer retries to avoid wasting time

#         # OPTIMIZATION 8: Prioritize creation of new documents
#         if to_create:
#             print(f"Creating {len(to_create)} new documents in batches")
...
#                                 # Don't log the entire doc_data which could be huge
#                                 print(f"Failed to create after {MAX_RETRIES} attempts: {e}")
#                             else:
#                                 time.sleep(0.2)  # Shorter pause

#                 create_end_time = time.time()
#                 if i % (BATCH_SIZE * 5) == 0:  # Log less frequently
#                     print(f"Created batch {i//BATCH_SIZE + 1}/{(len(to_create)-1)//BATCH_SIZE + 1} in {create_end_time - create_start_time:.2f}s")

#         # OPTIMIZATION 9: Only update essential documents if time is running short
#         # Check if we've used more than 70% of our expected available time
#         elapsed_time = time.time() - start_time if 'start_time' in locals() else 0
#         MAX_FUNCTION_TIME = 45  # Appwrite functions typically timeout around 60s

#         if elapsed_time > (MAX_FUNCTION_TIME * 0.7) and len(to_update) > 50:
#             # We're running out of time, only update the most important matches
#             # Prioritize matches in the near future or recently completed
#             print(f"Running low on time ({elapsed_time:.1f}s used). Prioritizing important updates.")
#             # Prioritize by date if available
#             if 'date' in df.columns and 'status' in df.columns:
#                 today = datetime.now().strftime('%Y-%m-%d')
#                 to_update = [update for update in to_update[:50]]

#         if to_update:
#             print(f"Updating {len(to_update)} existing documents in batches")

#             # Reduced logging for updates to save processing time
#             for i in range(0, len(to_update), BATCH_SIZE):
#                 batch = to_update[i:i+BATCH_SIZE]

#                 if i % (BATCH_SIZE * 10) == 0:  # Log much less frequently
#                     print(f"Updating batch {i//BATCH_SIZE + 1}/{(len(to_update)-1)//BATCH_SIZE + 1}")

#                 for doc_id, doc_data in batch:
#                     # No retries for updates to save time - most important is to get all done
#                     try:
...
#                         # Just skip failed updates without logging to save time
#                         pass
...
def poisson_match_probs(params):
    ...
    home_probs = (
        np.exp(-home_xg)
        * np.power(home_xg, np.arange(max_goals))
        / np.array([math.factorial(i) for i in range(max_goals)])
    )
    away_probs = (
        np.exp(-away_xg)
        * np.power(away_xg, np.arange(max_goals))
        / np.array([math.factorial(i) for i in range(max_goals)])
    )
...
def calculate_expected_goals(
    home_team, away_team, attack_params, defense_params, home_advantage=1.2
):
    """
...
def add_team_rolling_stats(df, num_previous_games=6, home_advantage=1.2):
    ...
    """
    print(
        f"Calculating rolling statistics based on the last {num_previous_games} games..."
    )
    ...
    for season in seasons:
        ...
        for team_id, matches in team_performance.items():
            ...
            recent_matches = (
                matches[-num_previous_games:]
                if len(matches) > num_previous_games
                else matches
            )
            ...
            avg_goals_scored = (
                sum(goals_scored) / len(recent_matches) if recent_matches else 1.5
            )
            avg_goals_conceded = (
                sum(goals_conceded) / len(recent_matches) if recent_matches else 1.5
            )
            ...
            # Store parameters
            attack_params[team_id] = (
                attack_alpha,
                1.0,
            )  # using fixed beta for simplicity
            defense_params[team_id] = (
                defense_alpha,
                1.0,
            )  # using fixed beta for simplicity
        ...
        for idx, row in season_df.iterrows():
            ...
            previous_home_matches = [
                match for match in home_team_history if match["match_date"] < match_date
            ]
            ...
            previous_home_matches = (
                previous_home_matches[-num_previous_games:]
                if previous_home_matches
                else []
            )
            ...
            home_goals_scored = sum(
                match["goals_scored"] for match in previous_home_matches
            )
            home_goals_conceded = sum(
                match["goals_conceded"] for match in previous_home_matches
            )

            # Calculate home team averages
            home_goals_scored_avg = (
                home_goals_scored / num_home_previous if num_home_previous > 0 else 0
            )
            home_goals_conceded_avg = (
                home_goals_conceded / num_home_previous if num_home_previous > 0 else 0
            )
            ...
            previous_away_matches = [
                match for match in away_team_history if match["match_date"] < match_date
            ]
            ...
            # Use the specified number of previous games or all available if less
            previous_away_matches = (
                previous_away_matches[-num_previous_games:]
                if previous_away_matches
                else []
            )
            ...
            away_goals_scored = sum(
                match["goals_scored"] for match in previous_away_matches
            )
            away_goals_conceded = sum(
                match["goals_conceded"] for match in previous_away_matches
            )

            # Calculate away team averages
            away_goals_scored_avg = (
                away_goals_scored / num_away_previous if num_away_previous > 0 else 0
            )
            away_goals_conceded_avg = (
                away_goals_conceded / num_away_previous if num_away_previous > 0 else 0
            )
            ...
            df.loc[idx, "home_team_goals_scored_average"] = round(
                home_goals_scored_avg, 2
            )
            df.loc[idx, "home_team_goals_conceded_average"] = round(
                home_goals_conceded_avg, 2
            )
            df.loc[idx, "away_team_goals_scored_average"] = round(
                away_goals_scored_avg, 2
            )
            df.loc[idx, "away_team_goals_conceded_average"] = round(
                away_goals_conceded_avg, 2
            )
            ...
            if (
                num_home_previous_all >= num_previous_games
                and num_away_previous_all >= num_previous_games
            ):
...
for season_id in SEASON_IDS:
    if season_id:  # Skip if collection ID is None or empty
        print(f"\nProcessing collection: {season_id}")
        documents = get_matches_from_db(season_id)
        ...
        if documents:
            ...
            df["source_collection"] = season_id

            # Add this dataframe to our list
            all_dataframes.append(df)
            print(f"Added {len(df)} rows from collection {season_id}")
        else:
            print(f"No documents found in collection {season_id}")
...
if all_dataframes:
    ...
    if "date_unix" in combined_df.columns:
        ...
        combined_df["date_unix"] = pd.to_numeric(
            combined_df["date_unix"], errors="coerce"
        )

        # Create a datetime column for sorting
        combined_df["datetime_temp"] = pd.to_datetime(
            combined_df["date_unix"], unit="s"
        )
    ...
    combined_df["winningTeamName"] = combined_df.apply(
        lambda row: row["home_name"]
        if row["winningTeam"] == row["homeID"]
        else row["away_name"]
        if row["winningTeam"] == row["awayID"]
        else "Draw"
        if row["winningTeam"] == 0
    ...
    combined_df = add_team_rolling_stats(
        combined_df,
        num_previous_games=NUM_PREVIOUS_GAMES,
...
def formatDataframe(df):
    ...
    if "full_time_result" in df.columns:
        ...
        print(
            "Column 'full_time_result' NOT found. Available columns:",
            df.columns.tolist(),
        )
        ...
        result_cols = [
            ...
            if "result" in col.lower() and "full" in col.lower()
        ]
        if result_cols:
...
def load_and_process_data(df):
    ...
    try:
        ...
        if missing_cols:
            ...
            if "home_team" in missing_cols:
                home_team_candidates = [
                    ...
                    and ("team" in col.lower() or "name" in col.lower())
                ]
        ...
        if (
            "full_time_result" not in df.columns
            and "home_goals" in df.columns
            and "away_goals" in df.columns
        ):
        ...
        if "home_team" in df.columns:
            unique_teams = set(df["home_team"].unique()).union(
...
def add_elo_xg_only(
    ...
    output_columns = ["home_xg_elo", "away_xg_elo"]
    ...
    if len(completed_matches) >= num_recent_games:
        ...
        print(
            f"Calculated league averages - Home xG: {league_avg_home_xg:.2f}, Away xG: {league_avg_away_xg:.2f}"
        )
    else:
        print(
            f"Using default league averages - Home xG: {league_avg_home_xg:.2f}, Away xG: {league_avg_away_xg:.2f}"
        )
    ...
    for season in df["season"].unique():
        ...
        teams_in_season = set(season_df["home_team"].unique()) | set(
        ...
        for match_idx in season_indices:
            ...
            away_elo_adjusted = (
                current_away_elo + home_advantage_elo
            )  # If they were home
            ...
            try:
                ...
                home_xg, away_xg = solve_for_xg(
                    target_probs, league_avg_home_xg, league_avg_away_xg
                )
                ...
                if home_strength_ratio > 1:  # Home team is stronger
                    df.at[match_idx, "home_xg_elo"] = round(
                        league_avg_home_xg * (home_strength_ratio**0.15), 2
                    )
                    df.at[match_idx, "away_xg_elo"] = round(
                        league_avg_away_xg * (1 / home_strength_ratio**0.1), 2
                    )
                else:  # Away team is stronger
                    df.at[match_idx, "home_xg_elo"] = round(
                        league_avg_home_xg * (home_strength_ratio**0.1), 2
                    )
                    df.at[match_idx, "away_xg_elo"] = round(
                        league_avg_away_xg * (1 / home_strength_ratio**0.15), 2
                    )

            # Update Elo ratings after the match if it has been played
            if match_idx in df.index and df.at[match_idx, "status"] in [
                "complete",
                "finished",
            ]:
                # Get match result
...
def _dixon_coles_loglikelihood(
    params, match_data, teams_attack, teams_defense, num_teams
):
    ...
    for home_idx, away_idx, home_goals, away_goals in match_data:
        ...
        home_rate = np.exp(
            teams_attack[home_idx] + teams_defense[away_idx] + home_advantage
        )
        away_rate = np.exp(teams_attack[away_idx] + teams_defense[home_idx])

        # Apply Dixon-Coles correction for low-scoring matches
        correction = _dixon_coles_correction(
            home_goals, away_goals, home_rate, away_rate, rho
        )

        # Poisson probability with Dixon-Coles correction
        home_prob = (
            np.exp(-home_rate) * (home_rate**home_goals) / np.math.factorial(home_goals)
        )
        away_prob = (
            np.exp(-away_rate) * (away_rate**away_goals) / np.math.factorial(away_goals)
        )
    ...
    for team_idx in range(num_teams):
        nll += regularization_strength * (
            teams_attack[team_idx] ** 2 + teams_defense[team_idx] ** 2
        )

    return nll


def _fit_dixon_coles_model(
    match_data, teams, initial_values=None, min_home_advantage=0.3
):
    """
    ...
    if initial_values is None:
        ...
        teams_attack = {
            teams_idx[team]: values["attack"]
            for team, values in initial_values.items()
            if team in teams_idx
        }
        teams_defense = {
            teams_idx[team]: values["defense"]
            for team, values in initial_values.items()
            if team in teams_idx
        }
    ...
    result = minimize(
        ...
        bounds=[
            ...
            (-1.0, 0.0),
        ],  # home advantage [min,1], rho [-1,0]
        options={"maxiter": 100},
    )

    # Extract optimized parameters
    home_advantage = max(
        result.x[0], min_home_advantage
    )  # Enforce minimum even if optimizer went below
    rho = result.x[1]
...
def calculate_expected_goals_DixonCole(
    home_team, away_team, attack_params, defense_params, home_advantage=1.3
):
    """
...
def add_dixon_coles_ratings(
    dataframe, num_recent_games=6, min_home_advantage=0.3, home_advantage_factor=1.3
):
    """
    ...
    for i, season in enumerate(seasons):
        ...
        fit_interval = max(
            num_recent_games,
            int(season_length / max(1, season_length / num_recent_games)),
        )
        ...
        for match_idx_position, match_idx in enumerate(season_indices):
            ...
            if enough_matches and (
                enough_new_matches or match_idx_position == len(season_indices) - 1
            ):
                print(
                    f"  Fitting model at match {match_idx_position + 1}/{len(season_indices)}"
                )
                ...
                for team in teams_in_season:
                    team_matches = _get_team_matches_dixonCole(
                        prev_season_matches, team
                    )

                    # Only use the most recent num_recent_games for each team
                    if len(team_matches) > 0:
                        recent_team_matches = team_matches.tail(
                            min(len(team_matches), num_recent_games)
                        )
                ...
                if len(match_data) >= (num_recent_games - 1) and len(teams_to_fit) >= (
                    num_recent_games - 1
                ):
                    ...
                    try:
                        ...
                        print(
                            f"  Model fit successful. Home advantage: {home_advantage:.2f}"
                        )
                    except Exception as e:
            ...
            if len(all_attack_values) >= 2:
                ...
                # Range for scaling
                attack_range = (
                    max_attack - min_attack if max_attack > min_attack else 1.0
                )
                defense_range = (
                    max_defense - min_defense if max_defense > min_defense else 1.0
                )

                # Scale to 0-3 range and shift to ensure minimum value is 0.5
                # For attack, higher raw value = better attack
                home_attack_scaled = (
                    0.5 + 2.5 * (current_home_attack - min_attack) / attack_range
                )
                away_attack_scaled = (
                    0.5 + 2.5 * (current_away_attack - min_attack) / attack_range
                )

                # For defense, lower raw value = better defense, so invert
                home_defense_scaled = (
                    0.5 + 2.5 * (max_defense - current_home_defense) / defense_range
                )
                away_defense_scaled = (
                    0.5 + 2.5 * (max_defense - current_away_defense) / defense_range
                )
                ...
                home_expected_goals, away_expected_goals = (
                    calculate_expected_goals_DixonCole(
                        home_team,
                        away_team,
                        attack_params,
                        defense_params,
...
def add_bradley_terry_ratings(dataframe, num_recent_games=6):
    ...
    bt_columns = ["home_xg_bt", "away_xg_bt", "bt_home_advantage"]
    ...
    for season in df["season"].unique():
        ...
        fit_interval = max(
            num_recent_games, len(season_df) // (len(season_df) / num_recent_games)
        )  # Fit approximately (len(season_df)/num_recent_games) times per season
        ...
        for match_idx_position, match_idx in enumerate(season_indices):
            ...
            enough_matches = (
                len(prev_season_matches) >= num_recent_games
            )  # Need matches for reliable Bradley-Terry: num_recent_games
            enough_new_matches = match_idx_position - last_fit_index >= fit_interval
            ...
            if enough_matches and (
                ...
                # For each team, get their most recent matches
                for team in teams_in_season:
                    team_matches = _get_team_matches_bradleyTerry(
                        prev_season_matches, team
                    )
                    ...
                    if len(recent_team_matches) > 0:
                        ...
                        for _, match in recent_team_matches.iterrows():
                            ...
                            if (
                                home_team_match in teams_idx
                                and away_team_match in teams_idx
                            ):
                                home_idx = teams_idx[home_team_match]
                                away_idx = teams_idx[away_team_match]
                ...
                if (
                    len(match_data) >= num_recent_games
                ):  # Ensure enough matches for optimization: num_recent_games
                    ...
                    initial_params = np.append(
                        initial_strengths,
                        bt_home_advantage if bt_home_advantage != 0 else 0.1,
                    )
                    ...
                    try:
                        result = minimize(
                            ...
                            bounds=[(-3, 3)] * len(teams_in_season)
                            + [(0, 1)],  # Bounds to prevent extreme values
                        ...
                        optimized_strengths = optimized_strengths - np.mean(
                            optimized_strengths
                        )
                        ...
                        if (
                            len(optimized_strengths) > 1
                            and np.std(optimized_strengths) > 0
                        ):
                            optimized_strengths = optimized_strengths / (
                                2 * np.std(optimized_strengths)
                            )
                        ...
                        print(
                            f"  Model fit successful. Home advantage: {bt_home_advantage:.2f}"
                        )
                    except Exception as e:
            ...
            current_home_bt = bt_ratings.get(
                home_team, 1.0
            )  # Default to average (1.0) if not available
...
def _calculate_pythagorean_expectation(goals_scored, goals_conceded, exponent=1.83):
    ...
    expectation = goals_scored**exponent / (
        goals_scored**exponent + goals_conceded**exponent
    )
    return expectation
...
def add_pythagorean_ratings(dataframe, num_recent_games=6, exponent=1.83):
    ...
    pyth_columns = ["home_xg_pyth", "away_xg_pyth", "home_team_luck_factor"]
    ...
    for i, season in enumerate(seasons):
        ...
        for match_idx_position, match_idx in enumerate(season_indices):
            ...
            home_team_matches = prev_season_matches[
                (prev_season_matches["home_team"] == home_team)
                | (prev_season_matches["away_team"] == home_team)
            ]
            away_team_matches = prev_season_matches[
                (prev_season_matches["home_team"] == away_team)
            ...
            if len(home_team_matches) > 0:
                ...
                if len(home_team_matches) <= min_matches_needed:
                    ...
                    recent_home_matches = home_team_matches.tail(
                        min(len(home_team_matches), num_recent_games)
                    )
                ...
                actual_win_pct = (home_team_wins + home_team_draws * 0.5) / len(
                    recent_home_matches
                )
                ...
                if home_team not in teams_with_ratings or match_idx_position % 100 == 0:
                    ...
                    print(
                        f"    Used for calculation: {len(recent_home_matches)} matches"
                    )
                    print(
                        f"    Goals: Scored={home_team_goals_scored}, Conceded={home_team_goals_conceded}"
                    )
                    print(
                        f"    Expected Win%={expected_win_pct:.3f}, Actual Win%={actual_win_pct:.3f}"
                    )
                    print(
                        f"    Strength={home_strength:.2f}, Luck={home_luck_factor:.2f}"
                    )
                ...
                if match_idx_position % 100 == 0:
                    print(
                        f"  Match {season_match_count}: {home_team} has no previous matches"
                    )
            ...
            if len(away_team_matches) > 0:
                ...
                if len(away_team_matches) <= min_matches_needed:
                    ...
                    # Get only the most recent matches
                    recent_away_matches = away_team_matches.tail(
                        min(len(away_team_matches), num_recent_games)
                    )
                ...
                # Calculate actual win percentage (including draws as half wins)
                actual_win_pct = (away_team_wins + away_team_draws * 0.5) / len(
                    recent_away_matches
                )
                ...
                if away_team not in teams_with_ratings and (
                    match_idx_position < 10 or match_idx_position % 100 == 0
                ):
                    print(f"    {away_team} matches so far: {len(away_team_matches)}")
                    print(
                        f"    Used for calculation: {len(recent_away_matches)} matches"
                    )
                    print(
                        f"    Goals: Scored={away_team_goals_scored}, Conceded={away_team_goals_conceded}"
                    )
                    print(
                        f"    Expected Win%={expected_win_pct:.3f}, Actual Win%={actual_win_pct:.3f}"
                    )
        ...
        print(
            f"  Teams with ratings: {len(teams_with_ratings)} of {len(teams_in_season)}"
        )

        # print match counts for each team to diagnose slow rating assignments
        print("\nTeam match counts:")
        sorted_teams = sorted(
            team_match_counts.items(), key=lambda x: x[1], reverse=True
        )
...
def _update_bayesian_parameters(
    ...
    if is_home:
        expected_goals_for = (
            attack_alpha * ((3.0 - opp_defense_alpha) / 3.0) * home_advantage
        )
        ...
        expected_goals_against = (
            opp_attack_alpha * ((3.0 - defense_alpha) / 3.0) * home_advantage
        )
    ...
    attack_likelihood = (
        poisson.pmf(goals_for, expected_goals_for) if expected_goals_for > 0 else 0.01
    )
    defense_likelihood = (
        poisson.pmf(goals_against, expected_goals_against)
    ...
    attack_alpha_new = (attack_alpha * attack_beta + (goals_for / 2.0)) / (
        attack_beta + 0.5
    )
    ...
    defense_adjustment = 3.0 / (
        goals_against + 1.0
    )  # Transforms goals against to a 0-3 scale value
    defense_alpha_new = (defense_alpha * defense_beta + defense_adjustment) / (
        defense_beta + 1
    )
    ...
    defense_alpha = (
        1 - smooth_factor
    ) * defense_alpha + smooth_factor * defense_alpha_new
...
def _calculate_match_probabilities(
    expected_home_goals, expected_away_goals, max_goals=10
):
    """
...
def _extract_team_strengths(attack_params, defense_params):
    ...
    attack_strengths = {
        team: min(max(strength, 0.0), 3.0)
        for team, strength in attack_strengths.items()
    }
    defense_strengths = {
        team: min(max(strength, 0.0), 3.0)
        for team, strength in defense_strengths.items()
    }

    return attack_strengths, defense_strengths


def add_bayesian_ratings(
    dataframe, num_recent_games=6, prior_strength=0.6, update_interval=6
):
    """
    ...
    for season in seasons:
        ...
        prior_attack, prior_defense, home_advantage, prior_precision = (
            _get_prior_parameters(df, prior_strength=prior_strength)
        )
        ...
        for match_idx_position, match_idx in enumerate(season_indices):
            ...
            if len(prev_season_matches) >= min_matches_needed:
                ...
                all_teams = set(prev_season_matches["home_team"].unique()) | set(
                    prev_season_matches["away_team"].unique()
                )
                ...
                for team in [home_team, away_team]:
                    team_matches = prev_season_matches[
                        (prev_season_matches["home_team"] == team)
                        | (prev_season_matches["away_team"] == team)
                    ]

                    # Sort by index (chronological order) and take the most recent matches
                    if len(team_matches) > 0:
                        team_recent_matches[team] = team_matches.sort_index(
                            ascending=False
                        ).head(num_recent_games)
                ...
                if match_idx_position - last_update_position >= update_interval:
                    ...
                    attack_strengths, defense_strengths = _extract_team_strengths(
                    ...
                    if match_idx_position % 100 == 0:
                        print(
                            f"  Updated model at match {match_idx_position + 1}/{len(season_indices)}"
                        )
                ...
                df.at[match_idx, "home_team_attack_bayesian"] = np.round(
                    attack_strengths.get(home_team, 1.5), 2
                )
                df.at[match_idx, "home_team_defense_bayesian"] = np.round(
                    defense_strengths.get(home_team, 1.5), 2
                )
                df.at[match_idx, "away_team_attack_bayesian"] = np.round(
                    attack_strengths.get(away_team, 1.5), 2
                )
                df.at[match_idx, "away_team_defense_bayesian"] = np.round(
                    defense_strengths.get(away_team, 1.5), 2
                )
                df.at[match_idx, "home_advantage_bayesian"] = np.round(
                    home_advantage, 2
                )
    ...
    for col in bayesian_columns:
        ...
        if (
            col != "home_advantage_bayesian"
        ):  # Skip home advantage which is fixed at 1.1
...
def _prepare_home_xg_regression_data(matches_df):
    ...
    if len(valid_matches) == 0:
        print("  No valid matches with non-zero home_xg_odds")
        return None, None
    ...
    for col in ["home_team_goals_scored_average", "away_team_goals_conceded_average"]:
        ...
        if valid_matches[col].isna().any():
            ...
            print(
                f"  Filling {valid_matches[col].isna().sum()} NA values in {col} with mean: {col_mean:.3f}"
            )
    ...
    for col in ["home_team_goals_scored_average", "away_team_goals_conceded_average"]:
        if valid_matches[col].std() < 1e-6:  # Almost zero variance
            print(f"  Warning: Near-zero variance in {col}, adding small random noise")
            valid_matches[col] = valid_matches[col] + np.random.normal(
                0, 0.01, len(valid_matches)
            )
    ...
    if len(valid_matches) > 1:
        ...
        if abs(correlation) > 0.997:  # Very high correlation
            print(
                f"  Warning: Features are highly correlated (r={correlation:.4f}), adding noise"
            )
            # Add small random noise to one feature
            valid_matches["home_team_goals_scored_average"] += np.random.normal(
                0, 0.01, len(valid_matches)
            )
    ...
    for col in [
        ...
        if invalid_mask.any():
            print(f"  Replacing {invalid_mask.sum()} invalid values in {col}")
    ...
    if valid_matches["home_xg_odds"].std() < 0.05:
        print("  Warning: Very low variance in target variable (home_xg_odds)")
    ...
    X = valid_matches[
        ["home_team_goals_scored_average", "away_team_goals_conceded_average"]
    ].values.astype(float)
    y = valid_matches["home_xg_odds"].values.astype(float)

    # Final validity check
    if not np.all(np.isfinite(X)) or not np.all(np.isfinite(y)):
        print("  Error: Non-finite values remain in data after preprocessing")
        return None, None

    # Debug info
    print(f"  Prepared home xG regression data: {len(X)} samples, X shape: {X.shape}")
...
def _prepare_away_xg_regression_data(matches_df):
    ...
    if len(valid_matches) == 0:
        print("  No valid matches with non-zero away_xg_odds")
        return None, None
    ...
    if len(valid_matches) > 1:
        ...
        if abs(correlation) > 0.997:  # Very high correlation
            ...
            # Add small random noise to one feature
            valid_matches["away_team_goals_scored_average"] += np.random.normal(
                0, 0.01, len(valid_matches)
            )
    ...
    if valid_matches["away_xg_odds"].std() < 0.05:
        print("  Warning: Very low variance in target variable (away_xg_odds)")
        # Add tiny noise to target to avoid perfect fit issues
        valid_matches["away_xg_odds"] += np.random.normal(0, 0.005, len(valid_matches))

    # Create feature matrix and target vector
    X = valid_matches[
        ["away_team_goals_scored_average", "home_team_goals_conceded_average"]
    ].values.astype(float)
    ...
    # Debug info
    print(f"  Prepared away xG regression data: {len(X)} samples, X shape: {X.shape}")
...
def _fit_time_weighted_regression(X, y, weights):
    ...
    try:
        ...
        print(f"  Statsmodels regression error: {e}")
        ...
        fallback_model = {
            ...
            "predict": lambda X_new: np.full(
                X_new.shape[0] if X_new.ndim > 1 else 1, mean_y
            ),
        }

        print(f"  Using fallback mean-based model (mean={mean_y:.3f})")
        return fallback_model


def predict_xg_with_model(
    model,
...
def add_time_weighted_xg_regression(
    dataframe, num_recent_games=6, time_decay_half_life=6
):
    """
    ...
    for i, season in enumerate(seasons):
        ...
        for match_idx_position, match_idx in enumerate(season_indices):
            ...
            away_goals_conceded_avg = df.at[
                match_idx, "away_team_goals_conceded_average"
            ]
            away_goals_scored_avg = df.at[match_idx, "away_team_goals_scored_average"]
            home_goals_conceded_avg = df.at[
                match_idx, "home_team_goals_conceded_average"
            ]
            ...
            if len(prev_season_matches) >= min_matches_needed:
                ...
                if (match_idx_position % time_decay_half_life == 0) or (
                    home_xg_model is None or away_xg_model is None
                ):
                    # Calculate time weights for previous matches
                    weights = _calculate_time_weights(
                        prev_season_matches.index, time_decay_half_life
                    )

                    # Prepare data for home expected goals regression
                    X_home, y_home = _prepare_home_xg_regression_data(
                        prev_season_matches
                    )
                    ...
                    if (
                        X_home is not None
                        and y_home is not None
                        and len(X_home) >= min_matches_needed
                    ):
                        try:
                            home_xg_model = _fit_time_weighted_regression(
                                X_home, y_home, weights[: len(X_home)]
                            )
                    ...
                    X_away, y_away = _prepare_away_xg_regression_data(
                        prev_season_matches
                    )
                    ...
                    if (
                        X_away is not None
                        and y_away is not None
                        and len(X_away) >= min_matches_needed
                    ):
                        try:
                            away_xg_model = _fit_time_weighted_regression(
                                X_away, y_away, weights[: len(X_away)]
                            )
...
# update_database(df_twr)

# return context.res.empty()


# Add this section to your existing script after the Dixon-Coles implementation

###################################################################################################
#########################     MASSEY RATING MODEL    ############################################


def _build_massey_matrix(matches_df, teams):
    """
    Build the Massey rating matrix and point differential vector.
    """
    n_teams = len(teams)
    team_to_idx = {team: i for i, team in enumerate(teams)}

    # Initialize the Massey matrix A and point differential vector b
    A = np.zeros((n_teams, n_teams))
    b = np.zeros(n_teams)

    # Build the matrix from match results
    for _, match in matches_df.iterrows():
        home_team = match["home_team"]
        away_team = match["away_team"]

        # Skip if either team is not in our teams list
        ...
        # Point differential (home_goals - away_goals)
        point_diff = match["home_goals"] - match["away_goals"]

        # Update matrix A (frequency of games)
        A[home_idx, home_idx] += 1  # Home team played one more game
        A[away_idx, away_idx] += 1  # Away team played one more game
        A[home_idx, away_idx] -= 1  # Home team played against away team
        A[away_idx, home_idx] -= 1  # Away team played against home team

        # Update vector b (cumulative point differential)
        b[home_idx] += point_diff  # Home team's point differential
        b[away_idx] -= point_diff  # Away team's point differential (opposite)

    return A, b, team_to_idx


def _solve_massey_ratings(A, b):
    """
    Solve the Massey rating system using least squares method.
    """
    n = A.shape[0]
    ...
    # Replace the last row with the constraint that sum of ratings = 0
    A_modified = A.copy()
    A_modified[-1, :] = 1.0  # Sum constraint: all ratings sum to 0
    b_modified = b.copy()
    b_modified[-1] = 0.0

    try:
        # Try to solve using standard linear algebra
        ratings = solve(A_modified, b_modified)
    except (LinAlgError, np.linalg.LinAlgError):
        try:
            # If that fails, use least squares with regularization
            regularization = 1e-6
            A_reg = A_modified + regularization * np.eye(n)
            ratings = solve(A_reg, b_modified)
        except (LinAlgError, np.linalg.LinAlgError):
            # If still failing, use iterative least squares
            ratings, _ = lsqr(A_modified, b_modified)[:2]

    # Center the ratings (ensure they sum to 0)
    ratings = ratings - np.mean(ratings)

    return ratings


def calculate_massey_expected_goals(
    home_team, away_team, team_ratings, home_advantage=0.3, league_avg_goals=1.3
):
    """
    Calculate expected goals for a match using Massey ratings.
    """
    # Get team ratings (default to 0.0 if not available)
    home_rating = team_ratings.get(home_team, 0.0)
    away_rating = team_ratings.get(away_team, 0.0)

    # Calculate expected goal differential based on rating difference
    rating_diff = home_rating - away_rating

    # Convert rating difference to expected goals
    scale_factor = 0.1
    expected_goal_diff = rating_diff * scale_factor + home_advantage

    # Calculate expected goals for each team
    expected_home_goals = league_avg_goals + (expected_goal_diff / 2.0)
    expected_away_goals = league_avg_goals - (expected_goal_diff / 2.0)

    # Ensure expected goals are within reasonable bounds (0.1 to 4.0)
    expected_home_goals = max(0.1, min(expected_home_goals, 4.0))
    expected_away_goals = max(0.1, min(expected_away_goals, 4.0))

    return expected_home_goals, expected_away_goals


def add_massey_ratings(
    dataframe, num_recent_games=6, min_home_advantage=0.3, home_advantage_factor=1.3
):
    """
    Add Massey rating columns to the football dataset.
    """
    df = dataframe.copy()

    if df is None or len(df) == 0:
        print("Failed to load data or empty dataframe. Exiting.")
        return None

    # Create a sequential index for chronological ordering
    df = df.reset_index(drop=True)

    # Initialize Massey rating columns
    massey_columns = [
        "home_xg_massey",  # Expected goals for home team based on Massey ratings
        "away_xg_massey",  # Expected goals for away team based on Massey ratings
    ]

    # Initialize all columns with default values
    for col in massey_columns:
        df[col] = 0.0
    ...
    for i, season in enumerate(seasons):
        print(f"Processing Massey ratings for season: {season}")
        season_df = df[df["season"] == season].copy()
        season_indices = season_df.index.tolist()

        # Initialize team ratings dictionary for this season
        team_ratings = {}
        ...
        # Initialize all teams with rating of 0
        for team in teams_in_season:
            team_ratings[team] = 0.0

        # Track when we last calculated ratings
        last_calculation_index = -1

        # Adjust calculation interval based on season length and number of teams
        season_length = len(season_indices)
        calculation_interval = max(
            num_recent_games, int(season_length / max(1, len(teams_in_season)))
        )

        # Minimum matches needed before we start calculating ratings
        min_matches_needed = max(len(teams_in_season) // 2, num_recent_games)
        ...
        for match_idx_position, match_idx in enumerate(season_indices):
            ...
            # Get all previous completed matches in this season up to this match
            prev_season_matches = season_df[
                (season_df.index < match_idx)
                & (season_df["status"].isin(["complete", "finished"]))
            ]

            # Check if we should recalculate ratings
            enough_matches = len(prev_season_matches) >= min_matches_needed
            enough_new_matches = (
                match_idx_position - last_calculation_index >= calculation_interval
            )
            is_last_match = match_idx_position == len(season_indices) - 1

            if enough_matches and (enough_new_matches or is_last_match):
                print(
                    f"  Calculating Massey ratings at match {match_idx_position + 1}/{len(season_indices)}"
                )
                last_calculation_index = match_idx_position

                try:
                    # Get teams that have played in recent matches
                    ...
                    for team in teams_in_season:
                        ...
                        if len(team_matches) > 0:
                            # Use the most recent matches for this team
                            recent_team_matches = team_matches.tail(
                                min(len(team_matches), num_recent_games)
                            )
                            teams_with_matches.add(team)

                            # Add these matches to our dataset
                            for _, match in recent_team_matches.iterrows():
                                match_tuple = (
                                    match["home_team"],
                                    match["away_team"],
                                ...
                                if match_tuple not in [
                                    ...
                                    teams_with_matches.add(match["home_team"])
                                    teams_with_matches.add(match["away_team"])

                    # Only calculate if we have enough teams and matches
                    teams_to_rate = list(teams_with_matches)

                    if len(teams_to_rate) >= 3 and len(recent_matches_data) >= 3:
                        # Create dataframe for Massey calculation
                        massey_df = pd.DataFrame(recent_matches_data)

                        # Build and solve Massey system
                        A, b, team_to_idx = _build_massey_matrix(
                            massey_df, teams_to_rate
                        )

                        if A.shape[0] > 0:
                            ratings_vector = _solve_massey_ratings(A, b)

                            # Update team ratings dictionary
                            for team, idx in team_to_idx.items():
                                if idx < len(ratings_vector):
                                    team_ratings[team] = ratings_vector[idx]

                            print(
                                f"  Successfully calculated ratings for {len(teams_to_rate)} teams"
                            )
                        else:
                            print("  Warning: Empty matrix for Massey calculation")
                    else:
                        print(
                            f"  Not enough data: {len(teams_to_rate)} teams, {len(recent_matches_data)} matches"
                        )

                except Exception as e:
                    print(f"  Error calculating Massey ratings: {e}")
                    # Keep existing ratings if calculation fails

            # Calculate expected goals using current Massey ratings
            try:
                # Calculate league average goals from recent matches
                if len(season_df) > 0:
                    recent_season_matches = season_df[season_df.index < match_idx]
                    if len(recent_season_matches) > 0:
                        league_avg_goals = (
                            recent_season_matches["home_goals"].mean()
                            + recent_season_matches["away_goals"].mean()
                        ) / 2.0
                    else:
                        league_avg_goals = 1.3  # Default
                else:
                    league_avg_goals = 1.3

                # Calculate expected goals
                home_expected_goals, away_expected_goals = (
                    calculate_massey_expected_goals(
                        home_team,
                        away_team,
                        team_ratings,
                        min_home_advantage,
                ...
                # Store the expected goals
                df.at[match_idx, "home_xg_massey"] = round(home_expected_goals, 2)
                df.at[match_idx, "away_xg_massey"] = round(away_expected_goals, 2)

            except Exception as e:
                print(f"  Error calculating expected goals for match {match_idx}: {e}")
                # Use default values if calculation fails
                df.at[match_idx, "home_xg_massey"] = 1.3
                df.at[match_idx, "away_xg_massey"] = 1.3

    print("Massey rating calculation completed")
    return df


# Example usage - Add this after your Dixon-Coles code:
print("\n" + "=" * 60)
print("ADDING MASSEY RATINGS")
print("=" * 60)

df_massey = add_massey_ratings(
    df_twr,  # Use the output from Dixon-Coles as input
    num_recent_games=NUM_PREVIOUS_GAMES,
    min_home_advantage=MIN_HOME_ADVANTAGE,
    home_advantage_factor=HOME_ADVANTAGE,
)


df_massey = formatDataframe(df_massey)

# Save the enhanced dataframe with only the xG columns
df_massey.to_excel(OUTPUT_FILENAME, index=False)
print(f"Data with only xG values saved to {OUTPUT_FILENAME}")
