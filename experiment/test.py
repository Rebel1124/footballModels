...
def main(context):
    ...
    def update_database(dataframe):
        """Update the Appwrite database with the latest data from the Excel file."""
        ...
        # context.log(f"Loading data from {EXCEL_FILE}...")
        # df = pd.read_excel(EXCEL_FILE)

        df = dataframe
        
        # Format the date column to ISO string
        ...
        # For integer fields, ensure they are actually integers
        integer_columns = [
        ...
        # Create match_id field if it doesn't exist
        ...
        context.log(f"Loaded {len(df)} records from Excel file.")
        
        # Fetch existing documents from the database
        context.log("Fetching existing documents from Appwrite...")
        try:
            existing_docs = {}
            ...
            while True:
                response = databases.list_documents(
                    database_id=MODEL_DATABASE_ID,  # Fixed parameter name
                    collection_id=NFD_MODEL_COLLECTION_ID,  # Fixed parameter name
                    queries=[
                        Query.limit(limit),
                        Query.offset(offset)
                    ]
                ...
                batch_docs = response['documents']
                total_docs += len(batch_docs)
                
                # Store documents by match_id for quick lookup
                for doc in batch_docs:
                    if 'match_id' in doc:
                        existing_docs[doc['match_id']] = doc['$id']
                
                context.log(f"Fetched {total_docs} documents so far...")
            ...
            for _, row in df.iterrows():
                ...
                if match_id in existing_docs:
                    ...
                    # Document doesn't exist, prepare for creation
                    to_create.append(doc_data)
            
            # Perform batch updates in chunks to avoid timeouts
            BATCH_SIZE = 25
            
            # Create new documents
            if to_create:
                context.log(f"Creating {len(to_create)} new documents...")
                for i in range(0, len(to_create), BATCH_SIZE):
                    batch = to_create[i:i+BATCH_SIZE]
                    context.log(f"Creating batch {i//BATCH_SIZE + 1}/{(len(to_create)-1)//BATCH_SIZE + 1}...")
                    ...
                    for doc_data in batch:
                        try:
                            databases.create_document(
                                database_id=MODEL_DATABASE_ID,  # Fixed parameter name
                                collection_id=NFD_MODEL_COLLECTION_ID,  # Fixed parameter name
                                document_id=ID.unique(),  # Fixed document ID generation
                                data=doc_data
                            )
                        except AppwriteException as e:
                            context.log(f"Error creating document: {e}")
                            context.log(f"Problematic data: {doc_data}")
            else:
                context.log("No new documents to create.")
            
            # Update existing documents
            if to_update:
                context.log(f"Updating {len(to_update)} existing documents...")
                for i in range(0, len(to_update), BATCH_SIZE):
                    ...
                    for doc_id, doc_data in batch:
                        try:
                            ...
                            context.log(f"Error updating document: {e}")
                            context.log(f"Document ID: {doc_id}")
                            context.log(f"Problematic data: {doc_data}")
            else:
                context.log("No documents to update.")
            
            context.log("Database update completed successfully!")
            
        except AppwriteException as e:
            context.log(f"Error updating database: {e}")
    ...
    def fit_linear_regression(X, y, weights=None):
        """
        Custom implementation of weighted linear regression using NumPy.
        ...
        --------
        dict
            Fitted model with 'coef_' and 'intercept_' attributes
        """
        if weights is None:
        ...
        XTX = X_weighted.T @ X_with_intercept
        XTy = X_weighted.T @ y
        ...
        # Create a model dict to mimic sklearn's interface
        model = {
            'intercept_': intercept,
            'coef_': coef
        }
        
        # Add a predict method to the model
        def predict(X_new):
            return model['intercept_'] + X_new @ model['coef_']
    ...
    def _fit_time_weighted_regression(X, y, weights):
        ...
        """
        # Fit weighted linear regression with our custom implementation
        model = fit_linear_regression(X, y, weights)
